...
export const update_profile = ({ updateProfile, id, setLoading }) => async (dispatch) => {
  try {
    setLoading(true);
    const config = {
      headers: {
    ...
    const response = await axios.put(`${url}/api/v1/client/profile/${id}`, updateProfile, config);
    dispatch({
